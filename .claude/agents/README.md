...
**Expanded view:**
```
[intake-agent] → [intake-brd-review-agent] → [design-api-agent] → [design-style-agent] → [design-wireframe-agent] → [type-generator-agent] → feature-planner → feature-planner → feature-planner → test-designer → test-generator → developer → code-reviewer
              INTAKE                                              DESIGN (conditional per manifest)                                      SCOPE             STORIES           REALIGN          TEST-DESIGN     WRITE-TESTS     IMPLEMENT      QA
          (2 agents, once)                                      (up to 4 agents, mandatory, once)                                      (all epics)        (per epic)        (per story)       (per story)      (per story)     (per story)   (per story)
```
